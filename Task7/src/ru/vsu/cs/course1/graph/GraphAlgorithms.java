...
public class GraphAlgorithms {
    ...
    public static void deleteAdjTree(Graph graph) {
        int k = 0;
        int flag = 0;
        int[][] adjArr = new int[graph.edgeCount()][2];
        ...
        // заполнил в массив все связи в графе
        int k2 = 0;
        for (int i = 0; i < graph.vertexCount(); i++){
            for (int j = 0; j < adjArr.length; j++) {
                if (adjArr[j][1] == i){
                    k2+=1;
                    if (k2>1){
                        System.out.println("Нужно удалить "+ i +" вершину.");
                    }
                }
        ...
        // если крайнее ребро повторяется в массиве, то значит его нужно удалить
